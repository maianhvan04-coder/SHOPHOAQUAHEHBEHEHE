...
module.exports.updateProduct = asyncHandler(async (req, res) => {
  ...
  const product = await productService.productAdminUpdate(req.authz, userId, req.params.id, req.body);
  res.json({
...
module.exports.adminGetByIdForEdit = asyncHandler(async (req, res) => {
  const data = await productService.adminGetByIdForEdit(req.params.id);
...
module.exports.getAllProducts = async (req, res) => {
  try {
    const { search, sort, page, limit, category } = req.query;

    const data =
      await productService.getAllProductsForClient({
        search,
        sort,
        page,
        limit,
...
module.exports.getTopBestSellerProductsService = async (_, res) => {
  try {

    const result = await productService.getTopBestSellerProductsService();
    if (result.EC !== 0) {
